...
public class AppoinmentService {
	...
	public ArrayList<Appinment> getAllAppoinment(String tblId) {
		...
		try {
			...
			while (rs.next()) {
				...
				appoinment.setDimg_path(rs.getString("tbl_docters.img_path"));
	...
	public ArrayList<Table> getAllTable() {
		...
		try {
			...
			while (rs.next()) {
				...
				table.setdImg_path(rs.getString("tbl_docters.img_path"));
	...
	public boolean setPatient(Appinment appoinment) {
		DbConnection db = new DbConnection();
		try {
			String sql = "UPDATE  tbl_appoinment set patient='" + appoinment.getP_id() + "',dsp='" + appoinment.getDsp()
					+ "' WHERE id='" + appoinment.getId() + "'";
			db.putData(sql);
	...
	public String getDImgpath(int id) {
		DbConnection db = new DbConnection();
		try {
			String sql = "SELECT tbl_docters.img_path, tbl_appoinment.id FROM tbl_appoinment Inner Join tbl_tables ON tbl_tables.tbl_id = tbl_appoinment.tbl_id Inner Join tbl_docters ON tbl_tables.d_id = tbl_docters.d_id where tbl_appoinment.id='"
					+ id + "'";
			ResultSet rs = db.getData(sql);
			while (rs.next()) {
				return rs.getString("tbl_docters.img_path");
			}
		} catch (Exception e) {
	...
	// ----------------
	public ArrayList<ApMail> getAllAppoinmentDetails(ApMail ap) {
		ArrayList<ApMail> appDeta = new ArrayList<ApMail>();
		DbConnection db = new DbConnection();
		try {
			String sql = "SELECT tbl_tables.date, tbl_patient.p_fname, tbl_patient.p_lname, tbl_appoinment.intervels, tbl_appoinment.patient, tbl_patient.p_id, tbl_appoinment.id, tbl_docters.d_fname, tbl_docters.d_lname,tbl_patient.p_email FROM tbl_appoinment Inner Join tbl_patient ON tbl_patient.p_id = tbl_appoinment.patient Inner Join tbl_tables ON tbl_tables.tbl_id = tbl_appoinment.tbl_id Inner Join tbl_docters ON tbl_docters.d_id = tbl_tables.d_id where tbl_appoinment.patient='"
					+ ap.getPatient() + "' and tbl_appoinment.id='" + ap.getaId() + "' ";
			ResultSet rs = db.getData(sql);
			while (rs.next()) {

				ApMail apmail = new ApMail();

				apmail.setDate(rs.getString("tbl_tables.date"));
				apmail.setP_fname(rs.getString("tbl_patient.p_fname"));
				apmail.setP_lname(rs.getString("tbl_patient.p_lname"));
				apmail.setIntervel(rs.getString("tbl_appoinment.intervels"));
				apmail.setPatient(rs.getInt("tbl_appoinment.patient"));
				apmail.setaId(rs.getInt("tbl_appoinment.id"));
				apmail.setD_fname(rs.getString("tbl_docters.d_fname"));
				apmail.setD_lname(rs.getString("tbl_docters.d_lname"));
				apmail.setPmail(rs.getString("tbl_patient.p_email"));
				appDeta.add(apmail);
				LOG.info("Sucessfully Loaded  AppoinmentDetails   ! ");

			}
			LOG.info("Sucessfully Completed Load All Appoinments ! ");

		} catch (Exception e) {
			LOG.warn("Exception in  getAllAppoinmentDetails()  -> CreateTimeIntervelService :" + e);
		}
